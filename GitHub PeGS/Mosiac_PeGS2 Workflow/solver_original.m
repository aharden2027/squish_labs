function particle = solver_original(particle, dsParams)

maskradius = dsParams.maskradius;
scaling = dsParams.scaling;
fitoptions = dsParams.fitoptions;
...
N = length(particle); 
    parfor n = 1:N
        ...
        if (particle(n).z > 0 )
            ...
            % Fitting functions
            func = @(par) fringe_pattern_original(z, par(1:z),par(z+1:z+z), beta(1:z), fsigma, rm, px, dsParams); 
            ...
            imgFit = fringe_pattern_original(z, forces, alphas, beta, fsigma, rm, px*(1/scaling), dsParams);
            
            % Redo force balance
            if dsParams.newtonizeOutput
                 [alphas,forces] = forceBalance(forces,alphas,beta);
            end
