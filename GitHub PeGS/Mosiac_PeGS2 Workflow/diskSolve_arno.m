%Addiional Comment:
%   Includes a mosiac image option that runs off of mosiac_contacts.mat file
%   needs the helper function dimensions.m
%   Updated by 8/1/2025
%
...
function out = diskSolve_arno(fileParams, dsParams, verbose)
...
%% Load particle data structure (mosiac or standard)
% directory: particle data location
if dsParams.mosaic
    fprintf("starting discsolve_mosaic/n")
    mosaicParticleFile = fullfile(fileParams.topDir, fileParams.contactDir, 'mosaic_contacts.mat');
    nFrames = 1;
else
    allFiles = dir(fullfile(fileParams.topDir, fileParams.contactDir, '*_contacts.mat'));
    % Filter out 'mosaic_contacts.mat' (case-insensitive)
    particledirectory = allFiles(~strcmpi({allFiles.name}, 'mosaic_contacts.mat'));
...
for frame = 1 : nFrames
    ...
    %% Save output
    if dsParams.mosaic
        savename = strrep('mosaic_contacts', '_contacts', '_solved');
    else
        savename = strrep(particledirectory(frame).name , '_contacts','_solved');
    end

    save(fullfile(fileParams.topDir, fileParams.solvedDir , savename), 'particle', '-v7.3')
    
    %% Dimension and Placing alterations to bigsynthimage in case of mosiac
    if dsParams.mosaic
        [yDim, xDim, minY, minX, maxR] = dimensions(mosaicParticleFile);
    end 
    ...
    for n=1:NN %for all particles
        ...
        if (particle(n).z > 0 )
            ...
            if dsParams.mosaic
                x = floor(particle(n).x - minX + maxR); %interger rounded x coordinate of the current particle (adjusted to global cords)
                y = floor(particle(n).y - minY + maxR); %interger rounded y coordinate of the current particle (adjusted to global cords)
            else
   ...
   hold off;
    ...
    if dsParams.mosaic
    [~, baseName, ~] = fileparts(mosaicParticleFile);
    savename = [strrep(baseName, '_contacts', ''), '_Synth.tif'];
    else
    [~, baseName, ~] = fileparts(particledirectory(frame).name);
    savename = [strrep(baseName, '_contacts', ''), '_Synth.tif'];
    end

    imwrite(bigSynthImg, fullfile(fileParams.topDir, fileParams.solvedDir, savename), "tif", "Compression", "lzw", "RowsPerStrip", 100); %save synthetic image
