...
class IntegratedGantryCameraGUI:
    ...
    def create_lightbox_tab(self, parent):
        ...
        self.lightbox_connect_button = tk.Button(settings_frame, text="Connect", command=self.connect_arduino_mega,
                                                bg='#4CAF50', fg='white', width=15)
        self.lightbox_connect_button.grid(row=0, column=2, rowspan=1, padx=10, pady=5)

        self.lightbox_disconnect_button = tk.Button(settings_frame, text="Disconnect", command=self.disconnect_arduino_mega,
        ...
        # RED color controls
        tk.Label(color_frame, text="Red RGB").grid(row=0, column=0, padx=5)

        self.red_r = tk.Entry(color_frame, width=4)
        self.red_r.insert(0, "255")
        self.red_r.grid(row=0, column=1)

        self.red_g = tk.Entry(color_frame, width=4)
        self.red_g.insert(0, "0")
        self.red_g.grid(row=0, column=2)

        self.red_b = tk.Entry(color_frame, width=4)
        self.red_b.insert(0, "0")
        self.red_b.grid(row=0, column=3)
        
        self.red_preview = tk.Label(color_frame, text=" ", width=4, bg="#ff0000", relief='groove')
        self.red_preview.grid(row=0, column=4, padx=10)

        # GREEN color controls
        tk.Label(color_frame, text="Green RGB").grid(row=1, column=0, padx=5)

        self.green_r = tk.Entry(color_frame, width=4)
        self.green_r.insert(0, "0")
        self.green_r.grid(row=1, column=1)

        self.green_g = tk.Entry(color_frame, width=4)
        self.green_g.insert(0, "255")
        self.green_g.grid(row=1, column=2)

        self.green_b = tk.Entry(color_frame, width=4)
        self.green_b.insert(0, "0")
        self.green_b.grid(row=1, column=3)

        self.green_preview = tk.Label(color_frame, text=" ", width=4, bg="#00ff00", relief='groove')
        self.green_preview.grid(row=1, column=4, padx=10)

        # Live update bindings
        for entry in [self.red_r, self.red_g, self.red_b]:
            entry.bind("<KeyRelease>", lambda event: self.update_preview_color('red'))
        for entry in [self.green_r, self.green_g, self.green_b]:
            entry.bind("<KeyRelease>", lambda event: self.update_preview_color('green'))

        # -------------------- Command Frame --------------------
        command_frame = tk.LabelFrame(parent, text="Commands", padx=10, pady=10)
        command_frame.pack(pady=10)

        tk.Button(command_frame, text="Turn RED", width=15, bg='#ff0000', fg='white',
                command=self.send_custom_red).pack(side=tk.LEFT, padx=5)

        tk.Button(command_frame, text="Turn GREEN", width=15, bg='#00ff00', fg='white',
                command=self.send_custom_green).pack(side=tk.LEFT, padx=5)
    ...
    def send_custom_red(self):
        try:
            r = int(self.red_r.get())
            g = int(self.red_g.get())
            b = int(self.red_b.get())
            command = f"SET_COLOR {r} {g} {b}"
            self.send_arduino_command(command)
        except ValueError:
            print("Invalid RED RGB values")

    def send_custom_green(self):
        try:
            r = int(self.green_r.get())
            g = int(self.green_g.get())
            b = int(self.green_b.get())
            command = f"SET_COLOR {r} {g} {b}"
            self.send_arduino_command(command)
        except ValueError:
            print("Invalid GREEN RGB values")
    ...
    def update_preview_color(self, color_type):
        try:
            if color_type == 'red':
                ...
                self.red_preview.config(bg=hex_color)
            elif color_type == 'green':
                r = int(self.green_r.get())
                g = int(self.green_g.get())
                b = int(self.green_b.get())
                hex_color = f'#{r:02x}{g:02x}{b:02x}'
                self.green_preview.config(bg=hex_color)
        except ValueError:
            # Ignore incomplete or invalid input (like empty fields)
            pass
    ...
    def connect_arduino_mega(self):
        ...
        try:
            ...
            self.lightbox_status_label.config(bg='#90EE90')  # light green
    ...
    def disconnect_arduino_mega(self):
    ...
    def take_both_photos(self):
        ...
        try:
            ...
            # ---------- RED: No filter ----------
            if self.arduino_mega:
                red_r = self.red_r.get()
                red_g = self.red_g.get()
                red_b = self.red_b.get()
                red_command = f"SET_COLOR {red_r} {red_g} {red_b}"
                self.send_arduino_command(red_command)
            ...
            if self.arduino_mega:
                green_r = self.green_r.get()
                green_g = self.green_g.get()
                green_b = self.green_b.get()
                green_command = f"SET_COLOR {green_r} {green_g} {green_b}"
                self.send_arduino_command(green_command)
                time.sleep(0.5)
