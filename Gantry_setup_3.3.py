...
5. Lightbox contol with adaptive color control 
6. Safety limits and confirmation checks
"""
...
class IntegratedGantryCameraGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Integrated Gantry, Camera, and Lightbox Control")
        ...
        self.camera_width_px = 7952
        self.camera_height_px = 5304
        self.fov_width_mm = 140.7  # 10 9/16 inches in mm
       ...
       # Lightbox (Arduino Mega) connection parameters
        self.lightbox_port = tk.StringVar(value="COM14")      
        self.lightbox_baud = tk.StringVar(value="9600")       
    ...
    def create_status_frame(self):
        ...
        tk.Label(status_frame, text="Lightbox:", bg='#f0f0f0', font=('Arial', 12)).grid(row=0, column=4, padx=5)
        self.lightbox_status_var = tk.StringVar()
        self.lightbox_status_var.set("Disconnected")
        self.lightbox_status_label = tk.Label(status_frame, textvariable=self.lightbox_status_var,
                                            bg='#ffcccb', width=20, font=('Arial', 12))
        self.lightbox_status_label.grid(row=0, column=5, padx=5)
    ...
    def create_lightbox_tab(self, parent):
        """Create lightbox tab"""
        # -------------------- Settings Frame --------------------
        settings_frame = tk.LabelFrame(parent, text="Connection Settings", padx=10, pady=10)
        settings_frame.pack(pady=10)

        tk.Label(settings_frame, text="Serial Port:", width=15, anchor='e').grid(row=0, column=0, padx=5, pady=5)
        port_entry = tk.Entry(settings_frame, textvariable=self.lightbox_port, width=10)
        ...
        baud_entry = tk.Entry(settings_frame, textvariable=self.lightbox_baud, width=10)
        baud_entry.grid(row=1, column=1, padx=5, pady=5)

        self.lightbox_connect_button = tk.Button(settings_frame, text="Connect", command=self.connect_lightbox,
                                                bg='#4CAF50', fg='white', width=15)
        self.lightbox_connect_button.grid(row=0, column=2, rowspan=1, padx=10, pady=5)

        self.lightbox_disconnect_button = tk.Button(settings_frame, text="Disconnect", command=self.disconnect_lightbox,
                                                    bg='#f44336', fg='white', width=15)
        self.lightbox_disconnect_button.grid(row=1, column=2, rowspan=1, padx=10, pady=5)

        tk.Button(settings_frame, text="Apply", command=self.apply_lightbox_settings,
                bg='#2196F3', fg='white', width=10, height=1).grid(row=2, column=0, columnspan=2, pady=5)

        # -------------------- Color Settings Frame --------------------
        color_frame = tk.LabelFrame(parent, text="RGB Color Settings", padx=10, pady=10)
        color_frame.pack(pady=10)

        # UNPOLARIZED
        tk.Label(color_frame, text="Unpolarized RGB").grid(row=0, column=0, padx=5)
        self.unpolarized_r = tk.Entry(color_frame, width=4)
        self.unpolarized_r.insert(0, "0")
        self.unpolarized_r.grid(row=0, column=1)

        self.unpolarized_g = tk.Entry(color_frame, width=4)
        self.unpolarized_g.insert(0, "50")
        self.unpolarized_g.grid(row=0, column=2)

        self.unpolarized_b = tk.Entry(color_frame, width=4)
        self.unpolarized_b.insert(0, "0")
        self.unpolarized_b.grid(row=0, column=3)

        self.unpolarized_preview = tk.Label(color_frame, text=" ", width=4, bg="#003300", relief='groove')
        self.unpolarized_preview.grid(row=0, column=4, padx=10)

        # POLARIZED
        tk.Label(color_frame, text="Polarized RGB").grid(row=1, column=0, padx=5)
        self.polarized_r = tk.Entry(color_frame, width=4)
        self.polarized_r.insert(0, "0")
        self.polarized_r.grid(row=1, column=1)

        self.polarized_g = tk.Entry(color_frame, width=4)
        self.polarized_g.insert(0, "255")
        self.polarized_g.grid(row=1, column=2)

        self.polarized_b = tk.Entry(color_frame, width=4)
        self.polarized_b.insert(0, "0")
        self.polarized_b.grid(row=1, column=3)

        self.polarized_preview = tk.Label(color_frame, text=" ", width=4, bg="#00ff00", relief='groove')
        self.polarized_preview.grid(row=1, column=4, padx=10)

        # -------------------- Command Frame --------------------
        command_frame = tk.LabelFrame(parent, text="Commands", padx=10, pady=10)
        command_frame.pack(pady=10)

        # Use initial RGB to set button colors
        def get_rgb_hex(r_entry, g_entry, b_entry):
            try:
                r = int(r_entry.get())
                g = int(g_entry.get())
                b = int(b_entry.get())
        ...
        unpolarized_hex = get_rgb_hex(self.unpolarized_r, self.unpolarized_g, self.unpolarized_b)
        polarized_hex = get_rgb_hex(self.polarized_r, self.polarized_g, self.polarized_b)

        self.unpolarized_button = tk.Button(command_frame, text="Unpolarized", width=15, bg=unpolarized_hex, fg='white',
                                    command=self.send_custom_unpolarized)
        self.unpolarized_button.pack(side=tk.LEFT, padx=5)

        self.polarized_button = tk.Button(command_frame, text="Polarized", width=15, bg=polarized_hex, fg='white',
                                    command=self.send_custom_polarized)
        self.polarized_button.pack(side=tk.LEFT, padx=5)

        tk.Button(command_frame, text="Turn OFF", width=15,
                command=lambda: self.send_arduino_command("OFF")).pack(side=tk.LEFT, padx=5)

        self.outline_button = tk.Button(command_frame, text="Draw Outline", width=15, bg=unpolarized_hex, fg='white',
                                        command=self.send_unpolarized_outline)
        self.outline_button.pack(side=tk.LEFT, padx=5)

        tk.Button(command_frame, text="Setup Pattern", width=15,
                command=lambda: self.send_arduino_command("SETUP")).pack(side=tk.LEFT, padx=5)

        # -------------------- Live update bindings --------------------
        for entry in [self.unpolarized_r, self.unpolarized_g, self.unpolarized_b]:
            entry.bind("<KeyRelease>", lambda event: self.update_preview_color('UNPOLARIZED'))

        for entry in [self.polarized_r, self.polarized_g, self.polarized_b]:
            entry.bind("<KeyRelease>", lambda event: self.update_preview_color('POLARIZED'))

    def send_custom_unpolarized(self):
        try:
            r = int(self.unpolarized_r.get())
            g = int(self.unpolarized_g.get())
            b = int(self.unpolarized_b.get())
            command = f"SET_COLOR {r} {g} {b}"
            self.send_arduino_command(command)
        except ValueError:
            print("Invalid UNPOLARIZED RGB values")

    def send_custom_polarized(self):
        try:
            r = int(self.polarized_r.get())
            g = int(self.polarized_g.get())
            b = int(self.polarized_b.get())
            command = f"SET_COLOR {r} {g} {b}"
            self.send_arduino_command(command)
        except ValueError:
            print("Invalid POLARIZED RGB values")
    
    def send_unpolarized_outline(self):
        try:
            r = int(self.unpolarized_r.get())
            g = int(self.unpolarized_g.get())
            b = int(self.unpolarized_b.get())
            command = f"OUTLINE {r} {g} {b}"
            self.send_arduino_command(command)
        except ValueError:
            print("Invalid OUTLINE RGB values")
  
    def update_preview_color(self, color_type):
        try:
            if color_type == 'UNPOLARIZED':
                r = int(self.unpolarized_r.get())
                g = int(self.unpolarized_g.get())
                b = int(self.unpolarized_b.get())
                hex_color = f'#{r:02x}{g:02x}{b:02x}'
                self.unpolarized_preview.config(bg=hex_color)
                self.unpolarized_button.config(bg=hex_color)
                self.outline_button.config(bg=hex_color)
            elif color_type == 'POLARIZED':
                r = int(self.polarized_r.get())
                g = int(self.polarized_g.get())
                b = int(self.polarized_b.get())
                hex_color = f'#{r:02x}{g:02x}{b:02x}'
                self.polarized_preview.config(bg=hex_color)
                self.polarized_button.config(bg=hex_color)
        except ValueError:
            pass
    ...
    def connect_lightbox(self):
        """Connect to Arduino Mega (Lightbox) using selected COM port and baud rate"""
        port = self.lightbox_port.get()
        baud = self.lightbox_baud.get()

        try:
            self.arduino_mega = serial.Serial(port, int(baud), timeout=1)
            time.sleep(2)  # Allow time for Arduino to reset
            
            #Update status UI
            self.lightbox_status_var.set("Connected")
            self.lightbox_status_label.config(bg='#c8e6c9')  # light green

            print(f"Lightbox connected on {port} at {baud} baud.")

        except Exception as e:
            self.arduino_mega = None
            self.lightbox_status_var.set("Connection Failed")
            self.lightbox_status_label.config(bg='#ffcccb')  # light red
            print(f"Failed to connect to Lightbox: {e}")

    def disconnect_lightbox(self):
        """Disconnect the Arduino Mega (Lightbox)"""
        try:
            if self.arduino_mega and self.arduino_mega.is_open:
                self.arduino_mega.close()

            self.arduino_mega = None
            self.lightbox_status_var.set("Disconnected")
            self.lightbox_status_label.config(bg='#ffcccb')  # light red

            print("Lightbox disconnected.")

        except Exception as e:
            print(f"Error disconnecting Lightbox: {e}")

    def apply_lightbox_settings(self):
        """Apply lightbox connection settings"""
        # If already connected, ask to disconnect first
        if self.arduino_mega and self.arduino_mega.is_open:
            if messagebox.askyesno("Already Connected", 
                                "Lightbox is already connected. Disconnect to apply new settings?"):
                self.disconnect_lightbox()
            else:
                return

        messagebox.showinfo("Settings Applied", 
                        f"Lightbox settings updated:\nPort: {self.lightbox_port.get()}\nBaud: {self.lightbox_baud.get()}")
    ...
    def take_both_photos(self):
        """Take two photos with Arduino LED indicators: one without filter (UNPOLARIZED), one with (POLARIZED)"""
        if not self.camera_connected:
            return
        ...
        try:
            ...
            # ---------- UNPOLARIZED: No filter ----------
            if self.arduino_mega:
                unpolarized_r = self.unpolarized_r.get()
                unpolarized_g = self.unpolarized_g.get()
                unpolarized_b = self.unpolarized_b.get()
                unpolarized_command = f"SET_COLOR {unpolarized_r} {unpolarized_g} {unpolarized_b}"
                self.send_arduino_command(unpolarized_command)
            ...
            # ---------- POLARIZED: With filter ----------
            self.send_camera_command("FILTER,ON")
            time.sleep(5) #time to prevent filter shaking

            if self.arduino_mega:
                polarized_r = self.polarized_r.get()
                polarized_g = self.polarized_g.get()
                polarized_b = self.polarized_b.get()
                polarized_command = f"SET_COLOR {polarized_r} {polarized_g} {polarized_b}"
                self.send_arduino_command(polarized_command)
                time.sleep(1.5)
